...
If (-not ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole(
    ...
    Write-Warning "â— This script must be run as Administrator. Relaunching..."
...
do {
    ...
    if (-not $driveLetter -or $driveLetter.Length -ne 1) {
        Write-Host "âŒ Invalid input. Please enter a single drive letter (Aâ€“Z)." -ForegroundColor Red
        continue
    }

    if (-not (Test-Path $aosPath)) {
        Write-Host "âŒ Folder '$aosPath' does not exist. Please enter the correct drive letter." -ForegroundColor Red
    }
...
if ($deployAll -match '^(Y|YES)$') {
    # --- Deploy all reports ---
    Write-Host "`nâ–¶ Deploying ALL reports..." -ForegroundColor Yellow
    ...
    try {
        ...
        Write-Warning "âŒ Error during deployment."
        Write-Log "âŒ ERROR: $($_.Exception.Message)"
    }
}
elseif ($deployAll -match '^(N|NO)$') {
    ...
    do {
        ...
        if (-not $modelName) {
            Write-Host "âŒ Model name cannot be empty. Please try again." -ForegroundColor Red
        ...
        if (-not $reportName) {
            Write-Host "âŒ Report name cannot be empty. Please try again." -ForegroundColor Red
        ...
        Write-Host "`nâ–¶ Deploying report: $fullReport from model: $modelName" -ForegroundColor Yellow
        ...
        try {
            ...
            Write-Log "âœ… Report '$fullReport' from model '$modelName' deployed successfully."
            $success = $true
        }
        catch {
            Write-Warning "âŒ Error deploying '$fullReport' from model '$modelName'. Try again."
            Write-Log "âŒ ERROR deploying '$fullReport' from '$modelName': $($_.Exception.Message)"
            $success = $false
        }
    ...
    Write-Host "âŒ Invalid choice. Please answer Y or N. Exiting..." -ForegroundColor Red
