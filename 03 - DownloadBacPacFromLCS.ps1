...
if ([string]::IsNullOrWhiteSpace($downloadUrl)) {
    ...
    Write-Host "No download URL provided. Aborting."
    exit 1
}
...
if ($folderDialog.ShowDialog() -ne [System.Windows.Forms.DialogResult]::OK) {
    ...
    Write-Host "No target folder selected. Aborting."
    exit 1
}
...
if ([string]::IsNullOrWhiteSpace($fileName)) {
    ...
    Write-Host "No filename provided. Aborting."
    exit 1
}
...
if (Test-Path $targetFilePath) {
    ...
    if ($overwrite.ToUpper() -ne "Y") {
        ...
        Write-Host "Download canceled by user."
        exit 0
    } else {
...
if (-not (Test-Path $azcopyExe)) {
    ...
    Write-Host "azcopy.exe not found at: $azcopyExe"
    exit 1
}
...
# Run AzCopy download
Write-Host ""
Write-Host "Starting download using AzCopy..."
...
try {
    ...
    Write-Host ""
    Write-Host "Download completed. File saved to: $targetFilePath"
    ...
    Write-Host "AzCopy failed: $($_.Exception.Message)"
    exit 1
}
