...
If (-not ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole(
    ...
    Write-Warning "âš  Relaunching script as Administrator..."
...
# --- Execute SQL Command ---
try {
    Write-Host "`nâ³ Executing index maintenance on AxDB (timeout set to 60 minutes)..." -ForegroundColor Cyan
    ...
    if ($result) {
        ...
        Write-Host "`nâœ… Procedure completed. Return Value: $returnVal" -ForegroundColor Green
        Write-Log "Stored procedure executed successfully. Return Value: $returnVal"
    } else {
        Write-Host "`nâš  No return value captured." -ForegroundColor Yellow
        Write-Log "âš  Procedure executed but returned no output."
    }
}
catch {
    Write-Warning "âŒ Error executing stored procedure."
