...
if (-not $env:XKTOOLS_SELF_HEALED) {
    ...
    (Get-Content $thisScript -Raw) -replace '[â€œâ€]', '"' -replace 'â€“', '-' | Set-Content -Encoding UTF8 -Path $thisScript
...
try {
    Write-Host "`nðŸŒ Downloading XKTools from GitHub..." -ForegroundColor Cyan
    Write-Log "Downloading ZIP from $zipUrl"
    Invoke-WebRequest -Uri $zipUrl -OutFile $zipFile -UseBasicParsing

    Write-Host "ðŸ¦ Extracting contents..." -ForegroundColor Cyan
    ...
    if (Test-Path $mainScript) {
        Write-Host "`nðŸš€ Launching XKTools Menu..." -ForegroundColor Green
        ...
        Write-Host "âŒ Could not find main menu script: $mainScript" -ForegroundColor Red
        Write-Log "ERROR: Menu script not found at $mainScript"
    }

} catch {
    Write-Host "âŒ An error occurred: $($_.Exception.Message)" -ForegroundColor Red
